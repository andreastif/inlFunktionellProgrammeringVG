...
fun main() {
    ...
    println(getAllReportingSantas(tomten))
    println(getAllReportingSantas(tomten).size)

    println(getAllReportingSantas(raven))
    println(getAllReportingSantas(raven).size)

    println(getAllReportingSantas(myran))
    println(getAllReportingSantas(myran).size)

    println(getAllReportingSantas(trotter))
    println(getAllReportingSantas(trotter).size)

    println(getAllReportingSantas(bladlusen))
    println(getAllReportingSantas(bladlusen).size)
...
fun getAllReportingSantas(santa : Tomte) : ArrayList<Tomte> {
    ...
    //här lägger vi till alla santas i listan som vi slutligen returnerar
        ...
        for (i in 0 until size) { //yttre loopens storlek (traverserar horisontiellt för santas med fler än 1 underliggande worker)
           val reportingSanta = getAllReportingSantas(santa.workers[i]) //anropa funktionen så vi kan addera santas till listan som returneras uppåt
